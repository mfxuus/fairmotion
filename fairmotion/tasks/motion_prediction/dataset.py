...
class Dataset(data.Dataset):
    def __init__(self, dataset_path, device, mean=None, std=None, precision=None):
    ...
    def __getitem__(self, index):
        ...
        if self.precision == 'half':
            src_seq = torch.Tensor(src_seq).to(device=self.device).half()
            tgt_seq = torch.Tensor(tgt_seq).to(device=self.device).half()
        else:
...
def get_loader(
    ...
    dataset = Dataset(dataset_path, device, mean, std, precision)
